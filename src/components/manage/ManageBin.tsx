...
// Interface ini yang bikin error IntrinsicAttributes kalau gak pas
...
export default function ManageBin({ isOpen, onClose, editData }: ManageBinProps) {
  ...
  useEffect(() => {
    if (editData) {
      ...
      setFormData({ id: "", name: "", location: "", capacity: 0 });
    }
  }, [editData, isOpen]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-[2px] p-4 font-sans">
      <div className="bg-white w-full max-w-lg rounded-[24px] shadow-2xl relative p-8">
        ...
        <div className="mb-6 text-left">
          ...
          <p className="text-sm text-blue-400 mt-1 italic">Input kapasitas sebagai level isi (Maks 100L)</p>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1 text-left">
            <label className="text-sm font-semibold text-emerald-600 ml-1">Kapasitas Isi (0 - 100L)</label>
            <input 
              type="number"
              max="100"
              min="0"
              className="w-full px-4 py-3 bg-gray-50 border border-emerald-100 rounded-xl text-sm font-bold focus:ring-2 focus:ring-emerald-500 outline-none transition-all"
          ...
          <div className="flex justify-end gap-3 pt-6">
            <button type="button" onClick={onClose} className="px-8 py-2.5 text-sm font-semibold text-gray-500 bg-white border border-gray-200 rounded-xl">Batal</button>
            <button type="submit" className="px-8 py-2.5 text-sm font-semibold bg-[#00D26A] text-white rounded-xl shadow-lg hover:bg-[#00b95d] transition-all">
              Simpan Data
            </button>
          </div>
        </form>
